...
class Trainer():
    ...
    def train(self):
        ...
        for i in range(self.num_iters_per_epoch):
            ...
            smpl_output, _ = self.generator(input_feat, input_pose, is_train=True)
            ...
            # 3D/2D keypoint loss
            kp_2d_loss.update(loss_dict['loss_kp_2d_short'].item(), input_feat.size(0))
            kp_3d_loss.update(loss_dict['loss_kp_3d_short'].item(), input_feat.size(0))
            kp_2d_accel_loss.update(loss_dict['loss_accel_2d_short'].item(), input_feat.size(0))
            kp_3d_accel_loss.update(loss_dict['loss_accel_3d_short'].item(), input_feat.size(0))
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, target in enumerate(self.valid_loader):
                ...
                smpl_output, _ = self.generator(input_feat, input_pose, is_train=False, J_regressor=J_regressor)
