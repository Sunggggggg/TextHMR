...
class Trainer():
    ...
    def train(self):
        ...
        for i in range(self.num_iters_per_epoch):
            ...
            global_smpl_output, smpl_output, pose_3d = self.generator(input_feat, input_pose, is_train=True)
            
            timer['forward'] = time.time() - start
            start = time.time()

            gen_loss, loss_dict = self.criterion(
                generator_outputs_init=global_smpl_output,
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, target in enumerate(self.valid_loader):
                ...
                global_smpl_output, smpl_output, pose_3d = self.generator(input_feat, input_pose, is_train=False, J_regressor=J_regressor)
