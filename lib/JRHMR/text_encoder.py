...
class LQTEncoder(nn.Module):
    def __init__(self, depth=3, embed_dim=512, mlp_hidden_dim=1024,
        ...
        self.motion_extract = CrossAttention(embed_dim, embed_dim, 36, attn_drop=attn_drop_rate, proj_drop=drop_rate)
    ...
    def forward(self, text_embed):
        ...
        motion_query = self.motion_query.tile(B, 1, 1)
        motion_query = self.motion_extract(motion_query, feature, feature)

        motion_query = self.transformer(motion_query)
        return motion_query

def get_model(learn_query=False, depth=2, length=16, embed_dim=512, mlp_hidden_dim=1024, h=8, drop_rate=0.2, drop_path_rate=0.2, attn_drop_rate=0.):
    if learn_query :
